...
/// This script handles all our debug information for our Time system
...
{
    private MainTimeScript _clToDBase;
    ...
    private void BasicDebugControls()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Z) && _bTodDebugOn == false)
            _bTodDebugOn = true;
        else if (Input.GetKeyDown(KeyCode.Z) && _bTodDebugOn == true)
            _bTodDebugOn = false;
        ...
        if (Input.GetKeyDown(KeyCode.Q) && _clToDBase.GetSet_fTimeMultiplier <= 9.5f)
            _clToDBase.GetSet_fTimeMultiplier += 20f;
        else if (Input.GetKeyDown(KeyCode.E) && _clToDBase.GetSet_fTimeMultiplier >= 0.5f)
            _clToDBase.GetSet_fTimeMultiplier -= 10f;
        else if (Input.GetKeyDown(KeyCode.R) && _clToDBase.GetSet_fTimeMultiplier >= 0.5f)
    ...
    void OnGUI()
    {
        ...
        if (_bTodDebugOn == true)
        {
            ...
            GUI.Label(new Rect(Screen.width / 2 - 120, 20, 240, 30), "CLOCK");
            GUI.Label(new Rect(Screen.width / 2 - 225, 40, 450, 30), "");

            // What are we debugging
            GUI.color = Color.red;

            // Current time *F0 means we show 0 of the floats decimals
            GUI.Label(new Rect(20, 60, 200, 30), "Current tttime:");
            GUI.Label(new Rect(220, 60, 25, 30), _clToDBase.Get_fCurrentHour.ToString("F0"));
